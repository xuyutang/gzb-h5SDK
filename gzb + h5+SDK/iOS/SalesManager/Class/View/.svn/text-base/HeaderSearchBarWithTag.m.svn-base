...
@implementation HeaderSearchBarWithTag
...
-(void)layoutSubviews{
    ...
    deLine.backgroundColor = [UIColor lightGrayColor];
    [self addSubview:deLine];
    if ([self.delegate respondsToSelector:@selector(setFirstStatus)]) {
        [self.delegate setFirstStatus];
    }
}
...
-(void)btnAction:(NSObject*) objct{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(HeaderSearchBarClickBtn:current:andWithTag:)]) {
        
        [_delegate HeaderSearchBarClickBtn:(int)btn.tag current:_currentIndex andWithTag:self.tag];
...
-(void) setBtnColor:(int)index{
    ...
    for (int i = 0; i < _buttons.count; i++) {
        ...
        if (i == index) {
            [item setTitleColor:WT_RED forState:UIControlStateNormal];
            icon.textColor = WT_RED;
        }
        else{
...
-(void) setColor:(int)index{
    if (index == -1) {
        for (int i = 0; i < _buttons.count; i++) {
            [((UIButton *)_buttons[i])setTitleColor:WT_BLACK forState:UIControlStateNormal];
            [((UILabel *)_icons[i]) setTextColor:WT_BLACK];
        }
    }else{
