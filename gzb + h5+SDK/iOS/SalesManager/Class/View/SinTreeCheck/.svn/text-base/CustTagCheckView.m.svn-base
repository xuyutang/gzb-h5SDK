...
@implementation CustTagTreeCheckCell
...
-(id)initWithStyleAndParent:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier andParentView:(CustTagCheckView*)andParentView sinTreeNode:(SinTreeCheckNode *) sinTreeNode{
    self = [self initWithStyleAndParent:style reuseIdentifier:reuseIdentifier andParentView:andParentView];
    if(self){
        self.node = sinTreeNode;
        [self resetView:self.node];
    }
    return self;
}
...
-(void) resetView:(SinTreeCheckNode *) stcn{
...
@implementation CustTagCheckView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _tagsArray = [[NSMutableArray alloc] initWithCapacity:0];
    }
    return self;
}
-(void)dealloc{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	CustTagTreeCheckCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
	if (cell == nil)
	{
		cell = [[CustTagTreeCheckCell alloc] initWithStyleAndParent:UITableViewCellStyleSubtitle
    ...
    if (node.parentId == 0) {
        ...
        cell.textLabel.backgroundColor = [UIColor colorWithWhite:0.f alpha:.25f];
        cell.backgroundColor = [UIColor colorWithWhite:0.f alpha:.25f];
    }else{
        cell.textLabel.backgroundColor = WT_CLEARCOLOR;
        cell.backgroundColor = WT_CLEARCOLOR;
		...
		if([_cells indexOfObjectIdenticalTo:node]!=NSNotFound) {
            if (indexToRemove <_cells.count) {
                for (int i = indexToRemove+1; i<_cells.count; i++) {
                    CustTagTreeCheckCell *cell = (CustTagTreeCheckCell *)[self cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
    ...
    if (_bSingle) {
        ...
        if (_selectedCount > 0) {
            ...
            if (node.checked) {
                ...
                if (selectedCount == 0) {
                    //检测选中的分组个数
                    if ([self getMaxSlectedCount] >= _maxCount) {
...
-(int) getMaxSlectedCount{
    int count = 0;
    for (int i = 0; i < _countArray.count; i++) {
        if ([_countArray[i] intValue] > 0) {
            count ++;
        }
    }
    return count;
...
-(void)finishedCheckNode:(SinTreeCheckNode *)node status:(BOOL)checked{
    ...
    if (checked) {
        ...
        if (!bAdded) {
            ...
            CustomerTag_Builder *tag = [(CustomerTag *)node.parent toBuilder];
            [tag clearTagValues];
            [tag addTagValues:node.department];
            CustomerTag *custTag = [tag build];
            if (![_tagsArray containsObject:custTag]) {
                [_tagsArray addObject:custTag];
            }
        }
        ...
        [tag addTagValues:node.department];
        CustomerTag *custTag = [tag build];
        [_tagsArray removeObject:custTag];
        NSLog(@"remove node : %@",node.text);
    }
