//
//  ResearchSearchViewController.m
//  SalesManager
//
...
//终端类型 终端名称 姓名
@interface ResearchSearchViewController ()<DepartmentViewControllerDelegate>{
...
@implementation ResearchSearchViewController
@synthesize delegate;
...
- (void)viewDidLoad
{
    ...
    if (tmp == nil || [tmp count]<1) {
        ...
        [c setCategory:[[tmpCategory build] retain]];
...
-(void)toSearch{
    MarketResearchParams_Builder* pbparam = [MarketResearchParams builder];
    
    [pbparam setCustomerCategoryArray:[[NSArray alloc] initWithObjects:currentCustomerCategory, nil]];
    if (currentTextField != nil && currentTextField.text.length>0) {
        ...
        [pbparam setCustomersArray:[[NSArray alloc] initWithObjects:[[cb build] retain], nil]];
    }
