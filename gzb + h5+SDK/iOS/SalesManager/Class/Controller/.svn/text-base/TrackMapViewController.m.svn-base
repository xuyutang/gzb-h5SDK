...
@interface TrackMapViewController ()<DatePicker2Delegate,HeaderSearchDateViewDelegate,UIWebViewDelegate,UIScrollViewDelegate, EGORefreshTableHeaderDelegate>{
    int selectIndex;
    GZBWebView* webView;
...
- (void)_loadMap{
    NSString *urlParam = [NSString stringWithFormat:TRACK_MAP_URL,currentDate,user.id,USER.userName,USER.password];
...
- (void)webView:(UIWebView *)_webView didFailLoadWithError:(NSError *)error{
    ...
    [MESSAGE showMessageWithTitle:NSLocalizedString(@"menu_fuction_tack", @"")
