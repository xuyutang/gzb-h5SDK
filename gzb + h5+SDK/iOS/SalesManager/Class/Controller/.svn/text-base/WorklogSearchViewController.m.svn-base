...
@interface WorklogSearchViewController ()<DepartmentViewControllerDelegate>{
    NSMutableArray *departmentArray;
    NSMutableArray *checkedDepartmentArray;
}

@end
@implementation WorklogSearchViewController
@synthesize delegate;
...
-(void)toSearch{
    WorkLogParams_Builder* bsp = [WorkLogParams builder];
    
    if (checkedDepartmentArray != nil && checkedDepartmentArray.count >0) {
        [bsp setDepartmentsArray:checkedDepartmentArray];
    }else{
        [bsp clearDepartments];
    }
    if (currentUser != nil) {
        [bsp setUsersArray:[[NSArray alloc] initWithObjects:currentUser, nil]];
    }
    [bsp setStartDate:fromTime];
    [bsp setEndDate:toTime];
    [bsp setPage:1];
    
    if (delegate != nil && [delegate respondsToSelector:@selector(didFinishedSearchWithResult:)]) {
        [delegate didFinishedSearchWithResult:bsp];
    }
    [self dismissModalViewControllerAnimated:YES];
...
-(void)userSearch:(UserSelectViewController *)controller didSelectWithObject:(id)aObject{
    currentUser = [(User *)aObject retain];
