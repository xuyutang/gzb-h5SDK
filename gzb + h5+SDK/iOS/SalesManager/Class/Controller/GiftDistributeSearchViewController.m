//
//  GiftDistributeSearchViewController.m
//  SalesManager
//
...
@interface GiftDistributeSearchViewController ()

@end

@implementation GiftDistributeSearchViewController
@synthesize delegate;
...
- (void)viewDidLoad
{
	...
	tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 15, 320, 340) style:UITableViewStyleGrouped];
    ...
    [btSearch setFrame:CGRectMake(8, 320+30, 305, 40)];
    ...
    if (tmp == nil || [tmp count]<1) {
        ...
        if ((APPDELEGATE).myLocation != nil){
            [customer setAddress:(APPDELEGATE).myLocation.address];
            [customer setLongitude:(APPDELEGATE).myLocation.longitude];
            [customer setLatitude:(APPDELEGATE).myLocation.latitude];
        }
        [customer setUser:(APPDELEGATE).currentUser];
        CustomerCategory_Builder *tmpCategory = [CustomerCategory builder];
        [tmpCategory setId:0];
        [tmpCategory setName:@"全部"];
        
        [customer setCategory:[[tmpCategory build] retain]];
...
-(void)toSearch{
    ...
    if (currentCustomer != nil){
        [pb setCustomerCategoryArray:[[NSArray alloc] initWithObjects:currentCustomer, nil]];
    }
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if ([LOCALMANAGER hasFunction:FUNC_APPROVE_DES])return 7;
    else return 6;
}
    ...
    switch (indexPath.row) {
        ...
        case 5:{
            if ([LOCALMANAGER hasFunction:FUNC_APPROVE_DES]){
                ...
                [self tableView:tableView didSelectRowAtIndex:6];
            }
        }
            break;
        case 6:{
    ...
    switch (indexPath.row) {
        ...
        case 5:{
            ...
            if ([LOCALMANAGER hasFunction:FUNC_APPROVE_DES]){
                ...
                return [self tableView:tableView cellForRowAtIndex:6];
            }
        }
            break;
        case 6:{
