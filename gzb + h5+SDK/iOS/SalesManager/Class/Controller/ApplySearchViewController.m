...
@interface ApplySearchViewController ()<DepartmentViewControllerDelegate>{
...
@implementation ApplySearchViewController
@synthesize delegate;
...
- (void)viewDidLoad {
   ...
   leftImageView.image = [UIImage imageNamed:@"ab_icon_back"];
    ...
    [btSearch setFrame:CGRectMake(8, 250+30, 305, 40)];
    ...
    if ([applyCategories count]>0) {
        [applyCategories removeAllObjects];
    }
    
    applyCategories = [[NSMutableArray alloc] initWithArray:[LOCALMANAGER getApplyCategories]];
    ApplyCategory_Builder *tmpCategory = [ApplyCategory builder];
    [tmpCategory setId:0];
    [tmpCategory setName:@"全部"];
    [applyCategories insertObject:[[tmpCategory build] retain] atIndex:0];
...
-(void)toSearch {
    ApplyItemParams_Builder* pbparam = [ApplyItemParams builder];
    if (currentApplyCategory!=nil && currentApplyCategory.id != 0) {
        [pbparam setApplyCategory:currentApplyCategory];
    }

    if (checkedDepartmentArray != nil && checkedDepartmentArray.count >0) {
        [pbparam setDepartmentsArray:checkedDepartmentArray];
    }else{
        [pbparam clearDepartments];
    }
    if (currentUser != nil) {
        [pbparam setUsersArray:[[NSArray alloc] initWithObjects:currentUser, nil]];
    }
...
-(void)closePicker {
    ...
    [UIView setAnimationDelegate:self];
    [UIView commitAnimations];
    if (currentApplyCategory != nil && ![currentApplyCategory.name isEqual:@""])
        [tableView reloadData];
}
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    return [applyCategories count];
}

- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    return ((CustomerCategory *)[applyCategories objectAtIndex:row]).name;
}

- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    currentApplyCategory = [applyCategories objectAtIndex:row];
}
