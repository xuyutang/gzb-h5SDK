...
#import "JavascriptConduit.h"
#import "GlobalConstant.h"


@interface JavascriptConduit ()
@property             JavascriptBridge *conduitBridge;
@property (readwrite) NSMutableDictionary *headers;
@end

//

@implementation JavascriptConduit
...
- (void)_init
{
    ...
    _conduitBridge  = [[JavascriptBridge alloc] init];
...
- (void)loadRequest:(NSURLRequest *)request
{
    ...
    dispatch_async(queue, ^{
        NSMutableURLRequest *req = [self generateMutableRequestWithRequest:request andHeaders:self.headers];
...
- (void)javascriptBridgeDidFinishLoad:(UIWebView *)webView{
    [self.delegate conduitDidFinishLoad:webView];
}
