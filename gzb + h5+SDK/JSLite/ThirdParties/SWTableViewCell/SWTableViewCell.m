...
- (void)setTableView:(UITableView *)tableView rowHeight:(CGFloat)height leftUtilityButtons:(NSArray *)leftUtilityButtons rightUtilityButtons:(NSArray *)rightUtilityButtons{
    ...
    self.leftUtilityButtons = leftUtilityButtons;
    self.height = height;
    self.containingTableView = tableView;
    self.highlighted = NO;
    [self initializer];
}
...
- (void)initializer {
    ...
    int y = 0;
    int width = 320;
    if (cellType == kCellTypeGroup) {
        x = 10;
        y = 2;
        width = 300;
    }
    
    UIScrollView *cellScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(x, y, CGRectGetWidth(self.bounds)-(x*2), _height)];
    cellScrollView.contentSize = CGSizeMake(width, _height);
    ...
    //NSLog(@"NSStringFromClass([[self.subviews objectAtIndex:0] class]) = %@",NSStringFromClass([[self.subviews objectAtIndex:0] class]));
...
- (void)layoutSubviews {
    ...
    self.cellScrollView.frame = CGRectMake(x, 0, CGRectGetWidth(self.bounds)-(x*2), _height);
