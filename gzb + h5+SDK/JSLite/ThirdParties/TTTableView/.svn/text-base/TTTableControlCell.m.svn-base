...
+ (CGFloat)tableView:(UITableView*)tableView rowHeightForObject:(id)object {
  ...
  if (!height) {
    if ([view isKindOfClass:[UITextView class]]) {
      ...
      CGFloat ttLineHeight = textView.font.lineHeight;
      height = ttLineHeight * 5;

    }else if ([view isKindOfClass:[UITextField class]]) {
      UITextField* textField = (UITextField*)view;
      CGFloat ttLineHeight = textField.font.lineHeight;
...
- (void)layoutSubviews {
  ...
  if ([TTTableControlCell shouldSizeControlToFit:_control]) {
    ...
    CGFloat contentWidth = self.contentView.frame.size.width - kControlPadding;
    ...
    if (!_control.frame.size.height) {
      [_control sizeToFit];
    }

    if ([TTTableControlCell shouldConsiderControlIntrinsicSize:_control]) {
      minX += contentWidth - _control.frame.size.width;
    ...
    _control.frame = CGRectMake(minX, floor(self.contentView.frame.size.height/2 - _control.frame.size.height/2),
                                contentWidth, _control.frame.size.height);
  }
}
