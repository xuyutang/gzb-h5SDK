...
#define kMessageBarImageIconImportant @"icon-important.png"
...
- (void)showNextMessage
{
    if ([_messageBarQueue count] > 0){
        ...
        UITapGestureRecognizer *gest = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickMessageBar:)];
...
- (void)clickMessageBar:(id)sender{
    _isOpened = true;
    if (_delegate != nil && [_delegate respondsToSelector:@selector(itemClicked:)]) {
        [_delegate itemClicked:_object];
        _delegate = nil;
    }
...
+ (UIColor*)backgroundColorForMessageType:(MessageBarMessageType)type
{
    ...
    switch (type) {
        ...
        case MessageBarMessageTypeInfo:
            ...
            break;
        case MessageBarMessageTypeImportant:
            backgroundColor = [UIColor colorWithRed:0.698 green:0.133 blue:0.133 alpha:kMessageBarAlpha]; // Firebrick
            break;
        default:
            break;
    }
...
+ (UIImage*)iconImageForMessageType:(MessageBarMessageType)type
{
    ...
    switch (type) {
        case MessageBarMessageTypeError:
            iconImage = [UIImage JSLiteImageNamed:kMessageBarImageIconError];
            break;
        case MessageBarMessageTypeSuccess:
            iconImage = [UIImage JSLiteImageNamed:kMessageBarImageIconSuccess];
            break;
        case MessageBarMessageTypeInfo:
            iconImage = [UIImage JSLiteImageNamed:kMessageBarImageIconInfo];
            break;
        case MessageBarMessageTypeImportant:
            iconImage = [UIImage JSLiteImageNamed:kMessageBarImageIconImportant];
            break;
        default:
            break;
    }
