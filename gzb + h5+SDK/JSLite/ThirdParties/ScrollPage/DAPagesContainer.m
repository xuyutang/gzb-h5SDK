...
{
    _topBarHeight = 44.;
    _topBarBackgroundColor = [UIColor whiteColor];//[UIColor colorWithPatternImage:[UIImage imageNamed:@"bg_pagecontainer"]];//[UIColor scrollViewTexturedBackgroundColor];///
    _topBarItemLabelsFont = [UIFont systemFontOfSize:12];
    _pageIndicatorViewSize = CGSizeMake(40., 2.);
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated
{
    ...
    if (abs(self.selectedIndex - selectedIndex) <= 1) {
        ...
        [UIView animateWithDuration:(animated) ? 0.3 : 0. delay:0. options:UIViewAnimationOptionBeginFromCurrentState animations:^
         {
             [previosSelectdItem setBackgroundImage:nil forState:UIControlStateNormal];
             [previosSelectdItem setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
             [nextSelectdItem setBackgroundImage:[UIImage createImageWithColor:WT_RED] forState:UIControlStateNormal];
             [nextSelectdItem setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
         } completion:nil];
    } else {
    ...
    if (delegate != nil && [delegate respondsToSelector:@selector(didSelected:)]) {
        [delegate didSelected:selectedIndex];
    }
