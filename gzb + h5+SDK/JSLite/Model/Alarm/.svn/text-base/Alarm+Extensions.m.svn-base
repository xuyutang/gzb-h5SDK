...
+ (NSMutableArray *) getAlarmsWithManagedObjectContext:(NSManagedObjectContext *)moc{
    NSMutableArray* alarms = [[[NSMutableArray alloc] init] autorelease];
    
    NSFetchRequest *request = [[NSFetchRequest alloc] initWithEntityName:[self entityName]];
    NSError *error = nil;
    
    [alarms addObjectsFromArray: [moc executeFetchRequest:request error:&error]];

    return alarms;
}

+ (void) clearAlarmsWithManagedObjectContext:(NSManagedObjectContext *)moc{
    NSFetchRequest *request = [[NSFetchRequest alloc] initWithEntityName:[self entityName]];
    NSError *error = nil;
    
    NSArray* result = [moc executeFetchRequest:request error:&error];
    for (id alarm in result) {
...
- (void) createAlarmNotification
{
    ...
    nextNotification.alertBody = self.title;
    nextNotification.alertTitle = @"您有以下工作需要完成：";
    nextNotification.alertAction = [[NSBundle mainBundle]objectForInfoDictionaryKey:@"CFBundleDisplayName"];
    NSDictionary *infoDic = [NSDictionary dictionaryWithObjectsAndKeys:self.aid,@"aid", nil];
    nextNotification.userInfo = infoDic;
...
- (void) cancelAlarmNotification{
    ...
    for (UILocalNotification *notify in notificaitons) {
        if ([[notify.userInfo objectForKey:@"aid"] isEqualToString:self.aid]) {
